...
- **介面系統**：公開網站、問卷與管理後台共用 `main.css` 的高對比設計變數與元件語意；視覺以冷白、石墨黑與科技青綠建立專業企業品牌，輔以少量電藍和訊號黃，透過精準格線、細邊框與低圓角表現新竹科技意象，不使用漸層、裝飾性光球或大面積圓角卡片。首頁依「主視覺與成果統計→進行中調查→代表團使命與主要參與入口→常見問題」組織，後台採企業 SaaS 工作區的緊密資訊密度。頁面內容在 JavaScript 或捲動觀察器未執行時仍必須可見，題目在 320px 寬度必須安全換行，互動控制需有清楚的鍵盤焦點與行動版觸控區。
