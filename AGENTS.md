...
- **介面系統**：公開網站、問卷與管理後台共用 `main.css` 的高對比設計變數與元件語意；視覺採 Bauhaus 構成主義與紙本手稿的混合語彙，以暖象牙紙張、近黑墨色及克制的紅、藍、黃幾何標記建立品牌，搭配粗框、硬位移陰影、低幅度不規則圓角與少量手寫註記，不使用漸層、柔和漂浮陰影、裝飾性光球或大面積圓角卡片。公開頁可提高表現強度，問卷維持紙張式易讀結構，後台則採較克制的建築資訊表風格。首頁依「主視覺與成果統計→進行中調查→代表團使命與主要參與入口→常見問題」組織。頁面內容在 JavaScript 或捲動觀察器未執行時仍必須可見，題目在 320px 寬度必須安全換行，互動控制需有清楚的鍵盤焦點與行動版觸控區。
