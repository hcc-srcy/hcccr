...
- **介面系統**：公開網站、問卷與管理後台共用 `main.css` 的高對比設計變數與元件語意；視覺採白色、炭黑、青綠、珊瑚與黃色的公共服務編輯風格，不使用漸層、裝飾性光球或大面積圓角卡片。首頁依「主視覺與成果統計→進行中調查→代表團使命與主要參與入口→常見問題」組織，後台採較緊密的工作區密度。頁面內容在 JavaScript 或捲動觀察器未執行時仍必須可見，題目在 320px 寬度必須安全換行，互動控制需有清楚的鍵盤焦點與行動版觸控區。
