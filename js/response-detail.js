(async function () {
  ...
  try {
    ...
    root.innerHTML = `
      <div class="admin-page-heading"><div><p class="breadcrumbs no-print"><a href="/admin/responses.html?form=${encodeURIComponent(form.id)}">回應分析</a><span>/</span><span>${code}</span></p><h2>${window.HCCCR.escapeHtml(form.title)}</h2><p>回應識別碼：${code}</p></div><div class="page-actions no-print"><button class="button button--secondary button--small" type="button" data-print><i data-lucide="printer"></i> 列印 / PDF</button><button class="button button--danger button--small" type="button" data-delete><i data-lucide="trash-2"></i> 刪除</button></div></div>
      ...
      <dl>${form.fields.map((field) => { const answer = submission.answers[field.id]; const display = Array.isArray(answer) ? answer.join("、") : answer || "未填答"; return `<div class="response-block"><dt>${window.HCCCR.escapeHtml(field.label)}</dt><dd>${window.HCCCR.escapeHtml(display)}</dd></div>`; }).join("")}</dl>`;
    ...
    root.querySelector("[data-delete]").addEventListener("click", async () => {
      ...
      window.location.href = `/admin/responses.html?form=${encodeURIComponent(form.id)}`;
    });
    window.lucide?.createIcons();
  } catch (error) {
    root.innerHTML = '<section class="success-panel"><h2>找不到這筆回應</h2><p>資料可能已刪除，或網址不正確。</p><a class="button button--secondary" href="/admin/dashboard.html">返回調查總覽</a></section>';
    console.error(error);
  }
})();
