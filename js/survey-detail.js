(async function () {
  ...
  function bindEvents() {
    ...
    responseForm.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        root.innerHTML = `<section class="success-panel"><span class="success-panel__icon"><i data-lucide="check"></i></span><h2>回答已送出</h2><p>謝謝你分享真實經驗。請記下回應識別碼，以便日後提出資料權利申請。</p><span class="reference-code">${referenceCode(submittedAt, saved.id)}</span><div><a class="button" href="/surveys">返回調查中心 <i data-lucide="arrow-right"></i></a></div></section>`;
        window.lucide?.createIcons();
  ...
  try {
    ...
    if (unavailable) {
      root.innerHTML = '<section class="success-panel"><span class="success-panel__icon" style="background:#6b7672"><i data-lucide="calendar-x"></i></span><h2>目前無法填寫</h2><p>此調查尚未開始、已截止或暫停開放。</p><a class="button button--secondary" href="/surveys">返回調查中心</a></section>';
      window.lucide?.createIcons();
      return;
    }
    render();
  } catch (error) {
    root.innerHTML = '<section class="success-panel"><span class="success-panel__icon" style="background:#b42318"><i data-lucide="file-question"></i></span><h2>找不到這份調查</h2><p>網址可能有誤，或調查已經移除。</p><a class="button button--secondary" href="/surveys">查看其他調查</a></section>';
    window.lucide?.createIcons();
    console.error(error);
  }
})();
