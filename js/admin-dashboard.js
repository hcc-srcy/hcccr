(async function () {
  ...
  try {
    ...
    table.innerHTML = forms.map((form) => {
      ...
      return `<tr><td><div class="table-title"><strong>${window.HCCCR.escapeHtml(form.title)} ${form.is_edited ? '<span class="tag">已編輯</span>' : ""}</strong><small>/${window.HCCCR.escapeHtml(form.slug || form.id)}</small></div></td><td>${visibility}</td><td><span class="tag ${form.is_open ? "tag--success" : ""}">${form.is_open ? "開放" : "關閉"}</span></td><td>${count}</td><td>${window.HCCCR.formatDate(form.updated_at)}</td><td><div class="table-actions"><a class="icon-button" href="${window.HCCCR.getSurveyHref(form)}" target="_blank" title="預覽"><i data-lucide="eye"></i></a><a class="icon-button" href="${window.HCCCR.appUrl("/admin/responses.html")}?form=${encodeURIComponent(form.id)}" title="回應"><i data-lucide="chart-no-axes-column"></i></a><a class="icon-button" href="${window.HCCCR.appUrl("/admin/builder.html")}?id=${encodeURIComponent(form.id)}" title="編輯"><i data-lucide="pencil"></i></a></div></td></tr>`;
    }).join("");
