...
本專案是新竹縣兒少諮詢代表（竹縣兒少代表團）的官方門戶與不定期兒少議題調查系統。前台提供固定網址問卷、隱私同意門檻、響應式填答介面與聯絡表單；後台提供問卷管理、收件匣、網站內容編輯、動態統計及 Excel 匯出。
...
- 完整 SEO 架構，包含 canonical、Open Graph、Twitter Card、JSON-LD、robots、部署時動態 sitemap 與 RSS 2.0；僅目前開放的公開問卷可被索引及訂閱。
- 首頁結構化資料以「新竹縣兒少諮詢代表」為正式名稱，並納入「新竹縣兒少代表」、「新竹縣兒童及少年諮詢代表」、「竹縣兒少代表團」、「竹縣兒少諮詢代表」與「兒少代表」等常用名稱。公開頁依代表介紹、兒童權利、議題調查、政策提案與聯絡等不同搜尋意圖，以自然語句涵蓋新竹縣政府、新竹縣政府社會處、新竹縣兒少政策、校園權益、兒童權利公約及公共參與；結構化資料同步描述各頁主題，不使用關鍵字堆砌或誤稱為縣府官方網站。
- 全站採「暖色系親子質感 × 文青紙感」視覺系統：暖紙色（米白／卡其）與中性暖棕黑占主要畫面，低飽和鼠尾草綠負責品牌與互動層級，赤陶粉僅用於重要行動或強調，暖沙黃只作少量標記。首頁以大字敘事和新竹實景作為單一主視覺；調查頁採安靜的單欄工作軸；管理後台以平面面板、清楚資料表和單一鼠尾草綠狀態建立可信賴的資訊密度。三套介面分別由 `main.css`、`surveys.css`、`admin.css` 維護，標題採 Noto Serif TC 襯線字，陰影一律柔和暈染、圓角統一放大，不使用硬位移色塊陰影或檔尾覆蓋層，並將旋轉與手寫元素限制在少數品牌時刻。所有介面保留鍵盤焦點、行動版觸控區、320px 題目換行與無 JavaScript 時的內容可見性。
...
建置腳本會依自訂網域或 GitHub Pages 子路徑重寫 canonical、Open Graph、JSON-LD、`robots.txt`、`sitemap.xml` 與 RSS 2.0 `feed.xml`。若 Supabase 公開設定可用，sitemap 與 RSS 會透過 `list_public_forms` RPC 加入目前開放的 `public` 問卷；密碼保護、未列出、未開始、已截止與暫停問卷一律不加入。RPC 暫時無法連線時仍會產生有效的靜態 sitemap 與 RSS，不會中斷部署。

公開頁面均提供 RSS 自動探索標記，訂閱網址為 `https://hcccr.bond/feed.xml`。RSS 只包含公開網站介紹與公開問卷的標題、說明及網址，不包含題目答案或聯絡訊息。
