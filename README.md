...
- 全站採「現代編輯設計 × 克制 Bauhaus × 少量人味註記」視覺系統：暖紙色與中性色占主要畫面，低飽和藍色負責品牌與互動層級，紅色僅用於重要行動或提醒，黃色只作少量標記。首頁以大字敘事和新竹實景作為單一主視覺；調查頁採安靜的單欄工作軸；管理後台以平面面板、清楚資料表和單一藍色狀態建立可信賴的資訊密度。三套介面分別由 `main.css`、`surveys.css`、`admin.css` 維護，不使用漸層、柔和漂浮陰影或檔尾覆蓋層，並將硬陰影、粗框、旋轉與手寫元素限制在少數品牌時刻。所有介面保留鍵盤焦點、行動版觸控區、320px 題目換行與無 JavaScript 時的內容可見性。
