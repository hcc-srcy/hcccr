# AGENTS.md - 新竹縣兒少諮詢代表網站 AI 開發與協作規範
...
本專案為 **新竹縣兒少諮詢代表（竹縣兒少代表團）** 之官方門戶與不定期兒少議題調查系統：
...
- **介面系統**：`main.css` 定義全站設計變數與公開元件，`surveys.css`、`admin.css` 分別負責問卷工作區與管理工作區；三份樣式須各自維持單一來源，不得在檔尾疊加另一套舊版覆蓋層。視覺採暖色系親子質感與文青紙感的混合語彙：暖紙色（米白／卡其）和中性暖棕黑占 75% 以上，低飽和鼠尾草綠是主要品牌及互動色，赤陶粉僅用於重要行動與強調，暖沙黃只作少量標記；同一區段原則上只容許一個主要視覺概念。標題使用 Noto Serif TC 襯線字，正文維持 Noto Sans TC；圓角統一放大（6／10／18px），陰影一律採柔和暈染（無偏移、模糊漸淡），不使用硬位移色塊陰影。一般卡片、表格、表單與後台面板應使用 1px 暖色邊界，避免濃重黑框；旋轉與手寫元素不得進入資料表、表單或管理介面。公開頁維持柔和但克制的編輯表現，問卷採安靜的單欄工作軸，後台以排版、間距、分隔線及單一鼠尾草綠狀態建立品牌。首頁依「主視覺與成果統計→進行中調查→代表團使命與主要參與入口→常見問題」組織。頁面內容在 JavaScript 或捲動觀察器未執行時仍必須可見，題目在 320px 寬度必須安全換行，互動控制需有清楚的鍵盤焦點與行動版觸控區。
