...
- **介面系統**：`main.css` 定義全站設計變數與公開元件，`surveys.css`、`admin.css` 分別負責問卷工作區與管理工作區；三份樣式須各自維持單一來源，不得在檔尾疊加另一套舊版覆蓋層。視覺採現代編輯設計、克制 Bauhaus 與少量手作註記的混合語彙：暖紙色和中性色占 75% 以上，低飽和藍色是主要品牌及互動色，紅色僅用於重要行動與錯誤，黃色只作少量標記；同一區段原則上只容許一個主要視覺概念。一般卡片、表格、表單與後台面板應使用 1–2px 邊界且不加陰影，硬位移陰影只保留給主視覺、重要行動及少數功能卡；旋轉與手寫元素不得進入資料表、表單或管理介面。不得使用漸層、柔和漂浮陰影、裝飾性光球或大面積圓角卡片。公開頁維持較高但克制的編輯表現，問卷採安靜的單欄工作軸，後台以排版、間距、分隔線及單一藍色狀態建立品牌。首頁依「主視覺與成果統計→進行中調查→代表團使命與主要參與入口→常見問題」組織。頁面內容在 JavaScript 或捲動觀察器未執行時仍必須可見，題目在 320px 寬度必須安全換行，互動控制需有清楚的鍵盤焦點與行動版觸控區。
